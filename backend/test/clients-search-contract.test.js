...
test('busqueda de clientes incluye nombre, empresa, BAN, telefono y email', () => {
  ...
  assert.match(listQueryBlock, /SELECT c\.id, c\.name, c\.business_name, c\.business_name AS company,\s*c\.email,/);
  assert.match(listQueryBlock, /ORDER BY \$\{clientOrderSql\}\s+LIMIT \$\$\{params\.length \+ 1\} OFFSET \$\$\{params\.length \+ 2\}/);
  assert.match(clientsRealSource, /const total = await conn\.query\(\s*`SELECT COUNT\(\*\)::int AS total FROM clients c \$\{whereClause\}`/);
