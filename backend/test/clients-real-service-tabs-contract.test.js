...
test('Clientes calcula tabs de servicio desde PostgreSQL con line_kind y respaldo del BAN', async () => {
  const source = await readFile(routePath, 'utf8');

  assert.match(source, /const \{ tab, q, service \} = req\.query/);
  assert.match(source, /const SERVICE_CLIENT_SQL =/);
  assert.match(source, /const SERVICE_KIND_SQL = \(subscriberAlias, banAlias\)/);
  assert.match(source, /SERVICE_MOBILE_SQL\('s_service', 'b_service'\)/);
